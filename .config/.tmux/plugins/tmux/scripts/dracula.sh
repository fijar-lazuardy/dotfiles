...
main()
{
  ...
  for plug in "${plugins_left[@]}"; do

    if [ $plug = "cpu-usage" ]; then
      IFS=' ' read -r -a colors <<< $(get_tmux_option "@dracula-cpu-usage-colors" "orange dark_gray")
      scripts="#($current_dir/cpu_info.sh)"
    ...
    if $show_powerline; then
      if $show_empty_plugins; then
        tmux set-option -ga status-left "#[fg=${!colors[0]},bg=${powerbg},nobold,nounderscore,noitalics]${right_sep}#[fg=${!colors[1]},bg=${!colors[0]}] $scripts "
      else
        tmux set-option -ga status-left "#{?#{==:$scripts,},,#[fg=${!colors[0]},nobold,nounderscore,noitalics]${right_sep}#[fg=${!colors[1]},bg=${!colors[0]}] $scripts }"
      fi
      powerbg=${!colors[0]}
    else
      if $show_empty_plugins; then
        tmux set-option -ga status-left "#[fg=${!colors[1]},bg=${!colors[0]}] $scripts "
      else
        tmux set-option -ga status-left "#{?#{==:$scripts,},,#[fg=${!colors[1]},bg=${!colors[0]}] $scripts }aaa"
      fi
    fi
  done
  ...
  for plugin in "${plugins[@]}"; do
    ...
    elif [ $plugin = "artist" ]; then
      IFS=' ' read -r -a colors <<<$(get_tmux_option "@dracula-playerctl-colors" "cyan dark_gray")
      script="#($current_dir/artist.sh)"
    elif [ $plugin = "song" ]; then
      IFS=' ' read -r -a colors <<<$(get_tmux_option "@dracula-playerctl-colors" "pink dark_gray")
      script="#($current_dir/song.sh)"
    elif [ $plugin = "album" ]; then
      IFS=' ' read -r -a colors <<<$(get_tmux_option "@dracula-playerctl-colors" "light_purple dark_gray")
      script="#($current_dir/album.sh)"
